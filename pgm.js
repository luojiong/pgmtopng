...
async function readUntilWhitespace(
  file,
  fileSize,
  offset
){
  ...
  while (idx < fileSize) {
    ...
    if (isCharWhitespace(currentByte[0]) || idx == fileSize - 1) {
      ...
      return [Buffer.from(currentData).toString(), idx + 1];
    }
...
async function readPixels(
  ...
  if (signature == SIGNATURES.P5) {
    for (let idx = offset; idx < fileSize; ) {
      ...
      const currentByte = Buffer.alloc(readingSize);

      // console.log(`readPixels :${idx} readingSize:${readingSize}`);
      await util.promisify(fs.read)(file, currentByte, 0, readingSize, idx);
      for (let i = 0; i < readingSize; i = i + pixelSize) {
        pixels.push(currentByte.readUIntBE(i, pixelSize));
      }
      idx = idx + readingSize / pixelSize;
    }
 ...
 async function readPgm(filePath) {
  ...
  const [rawSignature, signatureEnd] = await readUntilWhitespace(
    file,
    fileSize,
    0
  );
  ...
  const [width, widthEnd] = await readUntilWhitespace(
    file,
    fileSize,
    signatureEnd
  );
  const [height, heightEnd] = await readUntilWhitespace(
    file,
    fileSize,
    widthEnd
  );
  const [maxval, maxvalEnd] = await readUntilWhitespace(
    file,
    fileSize,
    heightEnd
  );
  ...
  console.log(`spent: ${now - time}`);

  return { signature, width, height, maxval, pixels };
}

 async function writePngFromPgm(
  pgmData,
  outPath,
  colorMasks
) {
  ...
  for (let y = 0; y < newfile.height; y++) {
    for (let x = 0; x < newfile.width; x++) {
      ...
      const colorMaskIndex = Math.floor(
        (Math.min(pixel, 254) / 255) * colorMasks.length
      );
